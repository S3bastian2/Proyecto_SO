// p4.c - lector P4 (abre/crea recursos; notifica listo via SEM_READY)
...
int main(void) {
    ...
    if (fd == -1) { perror("shm_open p4"); exit(EXIT_FAILURE); }

    struct stat st;
    if (fstat(fd, &st) == -1 || st.st_size == 0) {
        if (ftruncate(fd, sizeof(struct dato_compartido)) == -1) { perror("ftruncate p4"); close(fd); exit(EXIT_FAILURE); }
        created_shm = 1;
    }

    struct dato_compartido *dt = mmap(NULL, sizeof(*dt), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    if (dt == MAP_FAILED) { perror("mmap p4"); close(fd); exit(EXIT_FAILURE); }
    if (created_shm) memset(dt, 0, sizeof(*dt));

    sem_t *alive_p4 = create_or_open_sem(SEM_ALIVE_P4, 1);
    ...
    // BARRERA: indicar que P4 está listo
    sem_t *ready = create_or_open_sem(SEM_READY, 0);
    if (sem_post(ready) == -1) { perror("sem_post ready (p4)"); }

    // ---- LECTURA: P4 lee hasta recibir -2 ----
    int received_minus2 = 0;
    while (!received_minus2) {
        sem_wait(sem4);
        sem_wait(mutex);
        if (dt->last1_cons == 4 && dt->last2_cons == 4) {
            sem_post(mutex);
            sem_wait(p4_block);
            sem_wait(mutex);
        }
        ...
        if (val > 0) {
            ...
            dt->last2_cons = old1; dt->last1_cons = 4;
            if (old1 == old2 && old1 == 3) sem_post(p3_block);
            printf("%d\n", val);
        } else if (val == -2) {
            dt->flag_minus2 = 1;
            received_minus2 = 1;
    ...
    // esperar a que P3 confirme que recibió -1
    sem_wait(p3_done);

    // enviar -3 a P2 por pipe
    int wfd = open(PIPE_P2, O_WRONLY);
    if (wfd != -1) { int t = -3; write(wfd, &t, sizeof(t)); close(wfd); }
    printf("P4: -3 enviado, P4 termina\n");
